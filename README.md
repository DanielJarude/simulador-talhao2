...
- **PR #5f — timeline de datas REAIS clicável**: abaixo do 3D, **um botão por cena STAC** ("30 AGO" / "2026", tooltip com data completa + nuvens, scroll horizontal + setas ‹ ›, 5/10/20+ cenas), com **Play/Pause**, velocidade e camada na mesma barra. O destaque azul ("ATUAL") só muda no **COMMIT** da cena (preload nunca move a seleção); estados discretos por ponto (disponível/carregando/pronta/aplicada/erro); "⇥ Mais recente" = `dates[0]`; períodos 30d/60d/90d/6m/1a/Personalizado reconstroem os chips preservando o cache compatível. A timeline real continua construída **exclusivamente** de `GET /api/talhao/{farm_id}/dates` (`is_real=true`/`source=sentinel-cdse`).
...
schema, assets autenticados e o **pipeline 3D (PR #4 ↔ PR #5f)** — **265 testes** (1 skip por

...
# --- Descoberta de módulos: a camada de integrações externas vive em `Api/` ---
# `services` é um PACOTE DE NAMESPACE (PEP 420, sem __init__.py) distribuído
# fisicamente entre `Back/services` (serviços internos) e `Api/services`
# (serviços acoplados a provedores externos). Os nomes de módulo continuam
# EXATAMENTE os mesmos (`services.<nome>_service`); apenas o diretório de
# origem mudou. Sem este insert, `uvicorn main:app` iniciado de `Back/` não
# enxergaria `Api/` (nada de PYTHONPATH obrigatório no launcher).
_API_LAYER_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Api"
)
if _API_LAYER_DIR not in sys.path:
    sys.path.insert(0, _API_LAYER_DIR)
...
#: As páginas/JS do frontend vivem em `Front/`; os datasets nativos
#: `sentinel-21KXQ-*` continuam na RAIZ do repositório (fora do Git). Cada
#: grupo é resolvido contra a sua própria base — as URLs servidas não mudam.
FRONTEND_DIR = os.path.join(BASE_PROJECT_DIR, "Front")
_FRONTEND_ROOT_REAL = os.path.realpath(FRONTEND_DIR)
_SENTINEL_ROOT_REAL = os.path.realpath(BASE_PROJECT_DIR)
...
def serve_frontend(frontend_path: str):
    ...
    base_dir = FRONTEND_DIR if is_frontend_file else BASE_PROJECT_DIR
    base_real = _FRONTEND_ROOT_REAL if is_frontend_file else _SENTINEL_ROOT_REAL
    candidate = os.path.join(base_dir, path)
    real = os.path.realpath(candidate)
    inside_root = real == base_real or real.startswith(base_real + os.sep)

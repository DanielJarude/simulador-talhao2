...
RUN_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# `services` é um pacote de namespace (PEP 420) repartido entre `Back/services`
# (internos) e `Api/services` (provedores externos) — ambos entram no path.
sys.path.insert(0, os.path.join(RUN_DIR, "Back"))
sys.path.insert(0, os.path.join(RUN_DIR, "Api"))

# Carrega .env do backend ANTES de importar config (mesma mecânica do app).
try:
    from dotenv import load_dotenv  # type: ignore
    load_dotenv(os.path.join(RUN_DIR, "Back", ".env"))
except ImportError:
    pass
...
def main() -> int:
    ...
    if not cds.is_configured():
        ...
        print("Preencha CDSE_CLIENT_ID/CDSE_CLIENT_SECRET em Back/.env e tente de novo.")
